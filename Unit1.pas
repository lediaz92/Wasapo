     ...
     If (RDUsuarios.NRO=Num) then
      If not(RDUsuarios.En_Linea) then
       begin
        ...
        begin
          SelectedButton:=MessageDlg('El usuario ya se encuentra en línea, desea reconectarse?',mtCustom,[mbYes,mbCancel], 0);
...
begin
  ...
  then begin
    ...
    then
    begin
        ShowMessage('Debe seleccionar un número válido');
    end
    else begin
      LO_Hash.AbrirMe(MeUsuarios);
      ...
      if not (LO_Hash.ExisteNro(MeUsuarios,StrToInt(Number),PosUser))
        then begin
          ShowMessage('El número no existe');
        end
        else begin
          ...
          then begin
            ShowMessage('Todavía no hay registros de chat con este usuario');
          end
          else begin
            if not (User.En_Linea)
            then begin
              ShowMessage('El usuario no se encuentra en linea, recibirá su mensaje una vez que inicie sesión');
            end
            else begin
              ActualizarChats();
            end;
    ...
    begin
     ...
     If (ExisteChat) then
      begin
       ...
       If not(enLinea) then
        EPCDatosChat.Text:=NombreUsuarioB+' - Última conexión: '+UltConexion
       else
